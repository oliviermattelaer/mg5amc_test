    ...
    def get_custom_propa(self, propa, spin, id):
        ...
        if not propa.startswith('1'):
            ...
            if (spin_id % 2):
                ...
                numerator =  "(cmath.sqrt(P(-1,id)*P(-1,id)) + Mass(id))*UFP(1,id)*UFPC(2,id) + (cmath.sqrt(P(-1,id)*P(-1,id)) - Mass(id))*VFP(1,id)*VFPC(2,id)"
            else:
                #numerator =  "VFP(1,id)*VFPC(2,id)"
                numerator =  "(cmath.sqrt(P(-1,id)*P(-1,id)) - Mass(id))*UFP(1,id)*UFPC(2,id) + (cmath.sqrt(P(-1,id)*P(-1,id)) + Mass(id))*VFP(1,id)*VFPC(2,id)"
               
            #denominator = "(2*Tnorm(id)*TnormZ(id))*(P(-1,id)*P(-1,id) - Mass(id) * Mass(id) + complex(0,1) * Mass(id) * Width(id))"
            denominator = "(2*cmath.sqrt(P(-1,id)*P(-1,id))*(2*Tnorm(id)*TnormZ(id))*(P(-1,id)*P(-1,id) - Mass(id) * Mass(id) + complex(0,1) * Mass(id) * Width(id)))"
        elif propa == "1M":
            # shift and flip the tag if we multiply by C matrices
            spin_id = id
            if (id + 1) // 2 in self.conjg:
