        ...
        for line in open(identpath):
            ...
            if block in self:
                try:
                    ...
                    if block == "mass" and lhaid == [25]: 
                        logger.warning('You have set the Higgsmass in param_card.dat \"%s %s" to value %s, in case you have set the FeynHiggs paramters (e.g. in MSSM runs) it will be calculated by FEYNHIGGS and replaced'%(block, lhaid,value))
                except KeyError:
                    value =defaultcard[block].get(tuple(lhaid)).value
                    if block == "mass" and lhaid == [25]: 
                        logger.warning('You have not set the Higgsmass in param_card.dat \"%s %s", in case you have set the FeynHiggs paramters (e.g. in MSSM runs) it will be calculated by FEYNHIGGS anyway, otherwise it will be set to the default value %s'%(block, lhaid,value))
                        continue
                    if block == "mass" and lhaid == [6]: 
                        continue
                    if block == "mass" and lhaid == [9000006]: 
                        continue
                    if block == "yukawa" and lhaid == [6]: 
                        continue
