...
c
     ...
     &     ivirtpointsExcept
      ...
      parameter (nbadmax = 5)
      double precision cf, xn, epinv, epinv2, xl12, c_virt
      parameter(cf=4d0/3d0)
      parameter(xn=3d0)
      double precision dot, conv
      data nbad / 0 /
...
c Ellis-Sexton scale)
      ...
      xl12=dlog(2d0*dot(p(0,1), p(0,2))/qes2)
...
c$$$      virt_wgt = born_wgt * 
c$$$     1    ao2pi*xn*2d0*(
c$$$     .     -epinv*(epinv2-xl12)-0.5d0*xl12**2+11d0/6d0+0.5d0*pi**2
c$$$     .     -((11d0-2d0*nf/xn)*epinv-1d0)/6d0)
c exact mass dependent virtual amplitude
      virt_wgt = born_wgt * 
     1    ao2pi*xn*2d0*(
