...
C     
C     Process: g d~ > x0 d~ XGLU=1 WEIGHTED=3 QNP=1 [ real = QCD ] / t
C     Process: g u~ > x0 u~ XGLU=1 WEIGHTED=3 QNP=1 [ real = QCD ] / t
C     Process: g s~ > x0 s~ XGLU=1 WEIGHTED=3 QNP=1 [ real = QCD ] / t
C     Process: g c~ > x0 c~ XGLU=1 WEIGHTED=3 QNP=1 [ real = QCD ] / t
C     Process: g b~ > x0 b~ XGLU=1 WEIGHTED=3 QNP=1 [ real = QCD ] / t
C     
      IMPLICIT NONE
C     
C     CONSTANTS
...
c$$$        ENDIF
c$$$      ENDDO
c$$$      ANS=ANS/DBLE(IDEN) * AMPLOtotop(MH**2)
...
c     #-(P(2,1)-P(2,4))**2 -(P(3,1)-P(3,4))**2
      ...
      ANS = (g**2/4.d0/pi)**3*AMPqg(sman,uman,tman) ! t <-> u switched
     &*dsqrt(2.d0)/12.d0/Pi*mh2*AMPLO(mh2) * 2d0/9d0*MDL_GF


c     p1 <-> p2 switched for gq
c$$$      sman =   2d0 * dot(p(0,1), p(0,2))
c$$$      tman = - 2d0 * dot(p(0,2), p(0,4))
c$$$      uman = - 2d0 * dot(p(0,1), p(0,4))
c$$$
c$$$      ANS = AMPqg(sman,tman,uman)
c$$$      ANS = ANS * (g**2/(8d0*pi**2))**3*(32d0/3d0)/48d0
c$$$     f /0.60365910530599576d0 * AMPLOtotop(MH**2)
...
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,4),W(1,2),W(1,5),GC_5,AMP(1))
      JAMP(1)=-AMP(1)
      MATRIX_4 = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
