...
C         SET THE HELICITY FILTER
          IF(.NOT.FOUNDHELFILTER) THEN
            ...
            IF(ISZERO(ABS(ANS(1,0))+ABS(ANS(2,0))+ABS(ANS(3,0)),REF
     $       /DBLE(NCOMB),-1,-1)) THEN
              IF(NTRY.EQ.1) THEN
...
C                 Be looser for helicity check, bring a factor 100
                  DUMMY=ISSAME(ANS(1,0),HELSAVED(1,H),REF,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ISZERO(ABS(ANS(1,0))+ABS(ANS(2,0))+ABS(ANS(3,0))
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ISSAME(ANS(1,0),HELSAVED(1,ABS(GOODHEL(HELPICKED)
...
C     
      ...
      DATA (LOOPAMPORDERS(I),I=221,224) /    2,    2,    2,    2/
C     -----------
C     BEGIN CODE
C     -----------
