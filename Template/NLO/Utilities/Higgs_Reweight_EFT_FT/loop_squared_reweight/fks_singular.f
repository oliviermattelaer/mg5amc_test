...
c     Anything else
      ...
      if (type.eq.14) wgt_ME_tree(2,icontr)=
     &     wgt_ME_tree(2,icontr)*max(dble(ngluons),1d0) ! this is actually the virtual matrix element
      do i=1,nexternal
         do j=0,3
...
c$$$               if (ickkw.ne.-1)
c$$$     &              virt_wgt=virt_wgt-average_virtual*born_wgt*ao2pi
...
c$$$            bsv_wgt=bsv_wgt+virt_wgt_save
         ...
         wgt_ME_real=virt_wgt

c$$$         if (abrv(1:4).ne.'virt' .and. ickkw.ne.-1)
c$$$     &        bsv_wgt=bsv_wgt+average_virtual*born_wgt*ao2pi
