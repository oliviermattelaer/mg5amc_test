...
c Compile with makefile_rwgt
     ...
     &     ,compute_rwgt_wgt_Hev
      ...
      data  reweight_loop_squared_var /.true./
c
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      double precision alphas,wgt_loop_sq,pp(0:3,nexternal),pi,born_wgt
     $     ,es2
      external alphas
      parameter (pi=3.14159265358979323846d0)
...
c coefficients that multiply the log(muR) and log(muF) factors.
      ...
      subroutine sushi_loop_wrap(ES2,pp,born_wgt,virt_wgt)
      implicit none
      include 'nexternal.inc'
      include '../ML5lib_reweight/SubProcesses/coupl.inc'
     ...
     $        ,b_mass,dum_mc,Hmass,dum_pseudo,dum_ew)
      ...
      xl12=dlog(2d0*dot(pp(0,1),pp(0,2))/es2)
      epinv = 0d0
      epinv2= 0d0
      virt_wgt = born_wgt * 
     1    ao2pi*xn*2d0*(
