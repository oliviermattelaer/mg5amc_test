...
class CanonicalConfigTag(diagram_generation.DiagramTag):
    ...
    def vertex_from_link(cls, legs, vertex_id, model):
        ...
        #was before vert_ID=vertex_id[1][0]
...
class HelasWavefunction(base_objects.PhysicsObject):
    ...
    def get_external_helas_call_dict(self):
        ...
        if self.is_boson():
            return_dict['state_id'] = (-1) ** (self.get('state') == 1)
        else:
    ...
    def get_spin_state_number(self, flip=False):
        ...
        state_number = {4:-1 if not flip else 1, #incoming
                        5:1 if not flip else -1, #outcoming
                        3: 1, 1: 1, 2: 1}
...
#        print "foooor %i ="%HelasAmplitude.counter, fermion_factor, self.get('type')
    ...
    def sign_flips_to_order(fermions):
        ...
        myferms = copy.copy(fermions)

        for i in range(len(myferms) - 1):
            for j in range(i + 1, len(myferms)):
                if myferms[j] < myferms[i]:
                    myferms[i], myferms[j] = myferms[j], myferms[i]
                    nflips = nflips + 1
...
class HelasMultiProcess(base_objects.PhysicsObject):
    ...
    @classmethod
    def process_color(cls,matrix_element, color_information, gen_color=True, 
                                                            compute_loop_nc=None):
