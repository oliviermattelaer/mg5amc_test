...
class ProcessExporterFortran(object):
    ...
    def __init__(self, mgme_dir = "", dir_path = "", opt=None):
        ...
        #if 'cluster_local_path' not in self.opt:
        #    raise Exception
...
class ProcessExporterFortranSA(ProcessExporterFortran):
    ...
    def copy_v4template(self, modelname):
        ...
        shutil.copy(pjoin(self.mgme_dir, 'madgraph', 'iolibs', 'template_files', 'ipnext.f'), 
                    pjoin(self.dir_path, 'SubProcesses', 'ipnext.f'))
...
def ExportV4Factory(cmd, noclean, output_type='default'):
    ...
    if cmd.options['color_ordering']:
        ...
        if cmd._export_format in ['standalone', 'matrix']:
            return color_ordered_export_v4.ProcessExporterFortranCOSA(\
                cmd._mgme_dir, cmd._export_dir, opt)
        elif cmd._export_format in ['madevent'] and group_subprocesses:
            return color_ordered_export_v4.\
                   ProcessExporterFortranCOMEGroup(cmd._mgme_dir, 
                                                   cmd._export_dir, opt)
        elif cmd._export_format in ['madevent']:
            return color_ordered_export_v4.\
                   ProcessExporterFortranCOME(cmd._mgme_dir, cmd._export_dir,
                                              opt)
    ...
    def generate_subprocess_directory_v4(self, subproc_group,
        ...
        if not isinstance(subproc_group, group_subprocs.SubProcessGroup):
            raise base_objects.PhysicsObject.PhysicsObjectError,\
                  "%s object not SubProcessGroup" % type(subproc_group)
